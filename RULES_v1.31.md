...
### 1.4 Плотная строка + обрезка = буквы без диакритики
- Плакатный заголовок живёт в строке **0.9 кегля** (`.work-title`): 24-й кегль
  в коробке 21,6. Пока заголовок ничем не обрезан, буквы спокойно выходят за
  коробку и всё видно.
- Но `truncate` — это `overflow: hidden`, и первым пропадает то, что выше
  заглавной: **точки у Ё и бреве у Й**. «ВЛАДИСЛАВ БРЮХОВСКИХ, Ё-ТАКСИ»
  читалось как «Е-ТАКСИ» (Артём, 03.09.26). Снизу так же срезает хвосты Д, Ц, Щ.
- Правило: обрезанному заголовку возвращаем коробке рост и тут же снимаем его
  отрицательным полем — на экране ничего не двигается:
  ```css
  .work-title.truncate { padding-block: 0.16em; margin-block: -0.16em; }
  ```
- Проверять на кириллице с Ё и Й, а не на латинице: у латиницы в капсе
  диакритики нет, и баг не виден вовсе.
...
- **Высота** одинаковая: **`h-9` (36)**. Не h-10, не h-11. Если рядом стоят селект, фильтр и «Сбросить» — все h-9.

  Было `h-10` (40) до 03.09.26. Поменяли, потому что от эталона разъехались в
  обе стороны: кнопки кабинета доросли до 44, кнопки календаря сели на 36. Из
  двух живых размеров выбрали меньший — Артём: «в календаре кнопки чуть меньше
  по высоте, и это выглядит более сбалансированно».

  Исключение одно: кнопка, стоящая **в одном ряду с полями**, берёт рост поля
  (40) — `work-btn--row`. Внутри ряда высоты обязаны совпадать, иначе линия
  ряда ломается.

  Ряд с полями — это ряд, где рядом с кнопкой правда стоит поле ввода. Ряд
  «Сохранить · Отмена» **под** формой полем не считается: 03.09.26 `--row`
  висел на четырнадцати таких кнопках, и они торчали на четыре пикселя над
  соседями (Артём: «проверь кнопки»). В календаре `--row` осталась ровно на
  двух — «Загрузить запись» рядом со ссылкой и «Добавить» рядом с датой.
