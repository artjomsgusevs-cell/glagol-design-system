...
> **Читать вместе с [RULES_v2.md](RULES_v2.md).** Разделы 1–8 ниже — общий корень
> обеих веток вида и действуют без исключений. Разделы **9–13** (таблицы,
> дашборды, approval flow, доступы, демо-режим) описывают портал Камертона,
> которого нет с мая 2026: роли `sales`/`head`/`lead`, компоненты
> `UserEditModal` и `PendingApprovalScreen`, автотест по пути `bureau-portal-v1.3/`.
> Что из них осталось живым, переписано в v2 под настоящие роли бюро; сами
> разделы оставлены как след происхождения приёмов.
...
- **Подпись — в одну строку.** Не влезает — переписываем текст, а не переносим
  его. Кнопка в два ряда ломает высоту всего ряда, и соседние кнопки перестают
  быть одного роста. Ужимать шрифт или ставить `whitespace-nowrap` с обрезкой
  тоже нельзя: подпись должна читаться целиком.

  Хорошо: «Не сохранять», «Отключить», «Показать снова».
  Плохо: «Уйти без сохранения», «Подключить заново», «Обновить занятость».

  Проверять на 375 — там переносится первым.
